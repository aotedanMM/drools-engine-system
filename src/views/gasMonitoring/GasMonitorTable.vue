    ...
    <vue-seamless-scroll :data="listData" :class-option="classOption" class="seamless-warp ">
      <ul v-loading="loading" class="item seamless-table">
        <li v-for="(item,index) in listData" :key="index" class="seamless-row m_gasMonitorTable">
          ...
          <p v-if="item.createTime !== null" class="m_rowItem">{{ item.createTime | formatDate }}</p>
          <p class="m_rowItem">{{ item.terminalId }}</p>
          <p class="m_rowItem">
...
export default {
  ...
  data() {
    return {
      rowClass: 'm_rowItem',
      titleData: ['编号', '时间', '设备号', '一氧化碳(ppm)', '甲烷(%)', '氧气(%)', '温度(℃)', '湿度(%)', '地点'],
